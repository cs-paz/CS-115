...
def questifyAlt(str_list):
    '''Same as questify'''
    lst = [] #initialize list value, also works if str_list == []
...
def wordScoreLoop(S, scorelist):
    '''Same as wordScore'''
    total = 0 #variable that adds up all the values of each letter
    for i in range(len(S)):
        total += letterScoreLoop(S[i], scrabbleScores)
    return total
...
def wordsWithScoreLambda(dct, scores):
    ...
    return map(lambda w : [w, wordScoreLoop(w, scores)], dct)
...
def wordsWithScoreLoop(dct, scores):
    ...
    for i in range(len(dct)):
        lst += [[dct[i], wordScoreLoop(dct[i], scores)]]
    return lst


"""Test Functions
lstOfStrings = ['cat', 'dog', 'mushroom', 'hotdog', 'jumbo', ]
print(questify(lstOfStrings))
print(questifyAlt(lstOfStrings))
print(catenate(lstOfStrings))
print(catenateLoop(lstOfStrings))
print(letterScore('g', scrabbleScores))
print(letterScoreLoop('g', scrabbleScores))
print(wordScore("dog",scrabbleScores))
